...
def simulate(policy, mode, config, seed, max_rounds=2000):
    ...
    for round_no in range(max_rounds):
        ...
        seconds += config['round_seconds'] if cash else config['failed_round_seconds']
        if cash == 0:
...
def main():
    ...
    args = p.parse_args()
    from challenge_strategy import PAYOUT_PRIOR
    config = dict(payout_probabilities=PAYOUT_PRIOR, round_seconds=11.5,
                  failed_round_seconds=9.0, flip_seconds=4.7, cashout_seconds=7.75,
                  fail_seconds=3.125, tie_seconds=2.5, min_cashout=0)
    if args.round_seconds:
        config['round_seconds'] = args.round_seconds
    policy = TimePolicy(config['payout_probabilities'], **{
        k: config[k] for k in ('round_seconds', 'flip_seconds', 'cashout_seconds', 'fail_seconds', 'failed_round_seconds', 'tie_seconds')},
