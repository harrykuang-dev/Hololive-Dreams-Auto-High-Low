...
class TimePolicy:
    ...
    """
    def __init__(self, payout_probabilities, round_seconds=11.5,
                 flip_seconds=4.7, cashout_seconds=7.75, fail_seconds=3.125,
                 min_cashout=0, failed_round_seconds=9.0, tie_seconds=2.5):
        ...
        if any(not math.isfinite(v) or v <= 0 for v in (round_seconds, flip_seconds, cashout_seconds, fail_seconds, failed_round_seconds, tie_seconds)):
        ...
        self.failed_round_seconds = failed_round_seconds
        self.tie_seconds = tie_seconds
        ...
        self.values = np.zeros(TARGET // 20 + 1)
        self._build()

    def remaining_time(self, remaining):
        if remaining <= 0:
            return 0.0
        return float(self.values[min(len(self.values) - 1, math.ceil(remaining / 20))])
    ...
    def _build(self):
        ...
        for units in range(1, len(self.values)):
            gap = units * 20
            limit = gap + 180  # 20-coin grid: bank below 20,000
            amounts = set()
                ...
                for cash in sorted(amounts, reverse=True):
                    ...
                    if cash * 2 <= limit:
                        ...
                        for direction, w in enumerate(self.transitions):
                            ...
                            ca = (self.flip_seconds * (1-3/51) + self.tie_seconds * 3/51
                                  + w @ na + lose * self.fail_seconds) / (1 - 3 / 51)
                            cb = (w @ nb + lose) / (1 - 3 / 51)
                    ...
                    lines[cash] = a, b
                aa = ((1-self.probs[0])*self.round_seconds
                      + self.probs[0]*(self.failed_round_seconds+self.fail_seconds))
                bb = self.probs[0]
    ...
    def decide(self, coins, cash, deck, rank=None):
        ...
        def challenge(amount, current, counts, depth, replay=False):
            total = sum(counts)
            if not total:
                return float('inf')
            if current is None:
            ...
            for high in (high_count >= low_count,):
                result = self.tie_seconds if replay else self.flip_seconds
                for i, n in enumerate(counts):
                    if not n:
                        continue
                    after = list(counts); after[i] -= 1
                    if i == current:
                        ...
                        if depth > 1:
                            future = challenge(amount, current, tuple(after), depth - 1, replay=True)
                        else:
                            win = sum(m for j, m in enumerate(after) if (j > i if high else j < i))
                            valid = sum(after) - after[i]
                            p = win / valid if valid else 0.0
                            future = self.tie_seconds + p * value(amount * 2, current, tuple(after), 0) + (1-p) * restart
                    elif (i > current) == high:
